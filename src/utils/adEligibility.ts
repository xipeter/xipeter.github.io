...
export function getAdEligibility(pageType: PageType, hasContent: boolean = true): AdEligibilityConfig {
  const adEligiblePages: Record<PageType, { eligible: boolean; reason?: string }> = {
    ...
    articles: { eligible: false, reason: 'Hub page - thin content' },
    article: { eligible: true },
    news: { eligible: false, reason: 'Empty section' },
    'news-item': { eligible: false, reason: 'Empty section' },
    tools: { eligible: false, reason: 'Hub page - thin content' },
    tool: { eligible: true },
    promos: { eligible: false, reason: 'Hub page - thin content' },
    promo: { eligible: true },
    topics: { eligible: false, reason: 'Topic aggregation page' },
    topic: { eligible: false, reason: 'Topic page not implemented' },
